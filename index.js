const contentLetterSrart_actived = "Chúc mừng sinh nhật 18 tuổi của đồng chí Thái Linh Uyên" //Lời mở đầu cho bức thư
const mainContentLetter = "Chúc mừng 18 tuổi, tuổi cũ đã qua, tuổi mới lại đến xin kính chúc đồng chí Uyên ngày càng xinh đẹp, mau ăn chóng lớn, chúc đồng chí Uyên thành công phát đạt, vạn sự như ý, phát tài phát lộc, sống lâu trăm tuổi. Mong rằng tuổi mới sẽ mang đến cho đồng chí nhiều niềm vui, sức khỏe và may mắn. Hãy tận hưởng mỗi ngày và đạt được những mục tiêu trong tuổi mới này nhé!ahihi" //Nội dung của bức thư

// Gắn 1 đường link ảnh bất kì
let imgStart = document.querySelector(".myAI"); //Hình ảnh xuất hiện trong lời mở đầu của bức thư
imgStart.src = "./img/Screenshot (119)2.png";

// Gắn 1 link ảnh bất kì
let imgLetter = document.querySelector(".img");
imgLetter.src = "./img/Thiết kế chưa có tên (5).png"; //Hình ảnh xuất hiện trong nội dung của bức thư sau khi bức thư được viết ra hết
        ...
        splitContentLetterSrart_actived.forEach((val, index) => {
            setTimeout(() => {
                ...
                if (index == contentLetterSrart_actived.length - 1) {
                    setTimeout(() => {
                        document.querySelector(".recieve").setAttribute("style", "opacity: 1; transition: .10s") 
                    }, 1000)
                }
            }, 50 * index)
        })
...
function createLight(a) {
    ...
    const blurLv = [3, 4];
    const count = a;
    const allDefaultColor = ["#ec92c1", "yellow", "##8ab0d7", "red", "#ec5ca3"]
    ...
    for (var i = 0; i < count; i++) {
        ...
        var moveTime = Math.floor(Math.random() * 6) + 4;

        var div = document.createElement("div");
        div.classList.add = "snow";
        div.style.position = "absolute";
        div.style.backgroundColor = allDefaultColor[Math.floor(Math.random() * 5)]
        div.style.borderRadius = Math.floor(Math.random() * 10 + 20).toString() + "px"
